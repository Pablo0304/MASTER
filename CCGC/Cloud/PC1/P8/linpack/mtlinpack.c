...
OUTPUTFILE is passed as the first argument (optional)
NTIMES is passed as the second argument (optional)
...
{
	static REAL aa[DIM][DIM],a[DIM][DIMP],b[DIM],x[DIM];
	REAL cray,ops,total,norma,normx;
	REAL resid,residn,eps,t1,tm,tm2;
	REAL epslon(),second(),kf;
	static int ipvt[DIM],n,i,ntimes,info,lda,ldaa,kflops,k;
        FILE *outfile;
        time_t mytime;
	...
	fprintf(outfile,ROLLING);fprintf(outfile,PREC);fprintf(outfile,"Precision Linpack (%d)\n\n", ntimes);
        mytime = time(NULL);
        fprintf(outfile,"%s\n",ctime(&mytime));
...
/*     compute a residual to verify results.  */ 
	...
	printf("  %8.1f      %16.8e%16.8e%16.8e%16.8e\n",
   	...
   	fprintf(outfile,"    times are reported for matrices of order %5d\n",n);
	fprintf(outfile,"      dgefa      dgesl      total       kflops     unit");
	fprintf(outfile,"      ratio\n");

        times[2][0] = total;
        times[3][0] = ops/(1.0e3*total);
        times[4][0] = 2.0e3/times[3][0];
        times[5][0] = total/cray;

   	fprintf(outfile," times for array with leading dimension of%5d\n",lda);
	print_time(0, outfile);
        ...
        times[2][1] = total;
        times[3][1] = ops/(1.0e3*total);
        times[4][1] = 2.0e3/times[3][1];
        ...
        times[2][2] = total;
        times[3][2] = ops/(1.0e3*total);
        times[4][2] = 2.0e3/times[3][2];
	...
	for (i = 0; i < ntimes; i++) {
        ...
        times[0][3] = (second() - t1 - tm2)/ntimes;
        t1 = second();
	...
	for (i = 0; i < ntimes; i++) {
        ...
        times[1][3] = (second() - t1)/ntimes;
        total = times[0][3] + times[1][3];
        times[2][3] = total;
        times[3][3] = ops/(1.0e3*total);
        times[4][3] = 2.0e3/times[3][3];
        times[5][3] = total/cray;

	print_time(1, outfile);
	print_time(2, outfile);
	print_time(3, outfile);
        ...
        times[2][4] = total;
        times[3][4] = ops/(1.0e3*total);
        times[4][4] = 2.0e3/times[3][4];
        ...
        times[2][5] = total;
        times[3][5] = ops/(1.0e3*total);
        times[4][5] = 2.0e3/times[3][5];
	...
	times[2][6] = total;
	times[3][6] = ops/(1.0e3*total);
	times[4][6] = 2.0e3/times[3][6];
	...
	times[0][7] = (second() - t1 - tm2)/ntimes;
	t1 = second();
	for (i = 0; i < ntimes; i++) {
		dgesl(aa,ldaa,n,ipvt,b,0);
	}
	times[1][7] = (second() - t1)/ntimes;
	total = times[0][7] + times[1][7];
	times[2][7] = total;
	times[3][7] = ops/(1.0e3*total);
	times[4][7] = 2.0e3/times[3][7];
	...
	fprintf(outfile," times for array with leading dimension of%4d\n",ldaa);
	print_time(4, outfile);
	print_time(5, outfile);
	print_time(6, outfile);
	print_time(7, outfile);
	fprintf(outfile,ROLLING);fprintf(outfile,PREC);
	fprintf(outfile," Precision %5d Kflops ; %d Reps \n",kflops,ntimes);
...
/*----------------------*/ 
print_time (row, outfile)
int row;
FILE *outfile;
{
fprintf(outfile,"%11.2f%11.2f%11.2f%11.0f%11.2f%11.2f\n",   (double)times[0][row],
...
{
	int init, i, j;
        REAL lnorm =0.0, *norma=0.0;

	init = 1325;
        #pragma omp parallel for private (i, init, lnorm)
	for (j = 0; j < n; j++) {
		for (i = 0; i < n; i++) {
			init = 3125*init % 65536;
			a[lda*j+i] = (init - 32768.0)/16384.0;
			lnorm = (a[lda*j+i] > lnorm) ? a[lda*j+i] : lnorm;
		}
                if (*norma < lnorm) 
                   #pragma omp critical 
                   *norma  = (*norma < lnorm) ? lnorm : *norma;
	}
        *norma = lnorm;
	for (i = 0; i < n; i++) {
          b[i] = 0.0;
